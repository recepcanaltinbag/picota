...
def calculate_total_score(total_score_type, dist_type, max_z, mean_of_CompTns, std_of_CompTns, len_of_cycle, lst_ant, lst_is, lst_xe):
    ...
    if std_of_CompTns == 0:
        std_of_CompTns = 1  # Assign default value to avoid ZeroDivisionError
    ...
    # Optimize sum calculations using built-in sum() instead of loops
    if total_score_type == 0:
        antc = sum(lst_ant) if lst_ant else 0
        isc = sum(lst_is) if lst_is else 0
        xc = sum(lst_xe) if lst_xe else 0
        total_score = (antc + isc + xc) ** z_c_l

    elif total_score_type == 1:
        antc = 100 if len(lst_ant) > 0 else 0
        isc = 100 if len(lst_is) > 0 else 0
        xc = 100 if len(lst_xe) > 0 else 0
        total_score = (antc + isc + xc) ** z_c_l

    elif total_score_type == 2:
        antc = sum(lst_ant) if lst_ant else 0
        isc = 1 if len(lst_is) > 0 else 0
        xc = sum(lst_xe) if lst_xe else 0
        if xc + antc == 0:
            antc = 50
...
def make_blast_db(path_of_makeblastdb, db_input, db_output, db_type):
    pin_file = db_output + ".pin"
    if os.path.exists(pin_file):
        print(f"BLAST database already exists at {db_output}, skipping creation.")
        return
...
def blast_driver(path_of_makeblastdb, path_of_blastn, path_of_blastp, out_blast_folder, db_path, blast_query, r_type, info_prod_dict, threshold_blast):
    cycle_file_name = os.path.basename(db_path)
    db_output = os.path.join(out_blast_folder,"blast_temp", cycle_file_name)
    db_dir = out_blast_folder + "/db_dir"
    tempb_dir = out_blast_folder + "/blast_temp"
    db_input = db_path
    result_path = os.path.join(out_blast_folder,"blast_files", os.path.basename(blast_query) + "_" + r_type + ".out")
    print('Blast Driver')
    if not os.path.exists(db_dir):
        os.mkdir(db_dir)
    if not os.path.exists(tempb_dir):
        os.mkdir(tempb_dir)
    ...
    try:
        print(db_input, db_output)
        if is_protein_fasta(db_input):
            make_blast_db(path_of_makeblastdb, db_input, db_output, "prot")
            print('Time for blastx')
            run_blast(path_of_blastp, blast_query, db_output, result_path)
            print('End of blastx')
        else:
            make_blast_db(path_of_makeblastdb, db_input, db_output, "nucl")
...
def is_protein_fasta(fasta_path):
    dna_letters = set('ATGCNatgcn')
    for record in SeqIO.parse(fasta_path, 'fasta'):
        seq_set = set(record.seq.upper())
        if seq_set - dna_letters:
            return True  # Protein gibi gözüküyor
    return False  # Hepsi DNA karakterleri
...
def scoring_main(cycle_folder, picota_out_folder, \
    ...
    for cycle_file in cycle_files:
        if control_cycle_file(cycle_file):
            ...
            picota_out_for_cycle =  os.path.join(picota_temp_folder, os.path.basename(cycle_file).split('.')[0])
            ...
            for splitted_cycle in splitted_cycles:
                ...
                out_file_gbk =  os.path.join(prodigal_out_for_cycle, os.path.basename(splitted_cycle) + '.gbk')
                out_file_nuc =  os.path.join(prodigal_out_for_cycle, os.path.basename(splitted_cycle) + '.fasta')
                try:
                    ...
                    if os.path.exists(path_to_antibiotics):
                        r_type = 'Antibiotics'
                        cds_list.extend(blast_driver(path_of_makeblastdb, path_of_blastn, path_of_blastx, out_blast_folder, path_to_antibiotics, out_file_nuc, r_type, info_prod_dict, threshold_blast=80))
                    else:
                        print('No available Antibiotics DB, control the path')
                    #Xenobiotics
                    if os.path.exists(path_to_xenobiotics):
                        r_type = 'Xenobiotics'
                        cds_list.extend(blast_driver(path_of_makeblastdb, path_of_blastn, path_of_blastx, out_blast_folder, path_to_xenobiotics, out_file_nuc, r_type, info_prod_dict, threshold_blast=80))
                    else:
                        print('No available Xenobiotics DB, control the path')
                    #Insertion Sequences
                    if os.path.exists(path_to_ises):
                        r_type = 'InsertionSequences'
                        cds_list.extend(blast_driver(path_of_makeblastdb, path_of_blastn, path_of_blastx, out_blast_folder, path_to_ises, splitted_cycle, r_type, info_prod_dict, threshold_blast=80))
                    else:
