...
def cycle_anaylsis(path_to_data, out_cycle_file, find_all_path, path_limit, min_size_of_cycle, max_size_of_cycle, name_prefix_cycle, min_component_number, max_component_number, k_mer_sim, threshold_sim):
    ...
    for path in selected_paths:
        ...
        if (int((i_count)/len(selected_paths)*100))%10 == 0:
            print('.',end='',flush=True)
        
        cycle_inf_obj = cycle_info(path, node_dict, edge_dict, cycle_info_list)
    ...
    print('\nReverse Oriented Composite Transposon Paths reducing:')
    for path in GW.reverseallPaths:
        i_count += 1

        if (int((i_count)/len(GW.reverseallPaths)*100))%10 == 0:
            print('.',end='',flush=True)
